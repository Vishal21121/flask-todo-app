...
def addUser(schema, collection):
    ...
    try:
        ...
        if user is None:
            ...
            userInsertedVal = collection.insert_one(
                {
                    "name": data["name"],
                    "email": data["email"],
            ...
            return (
                jsonify(
                    {"status": "success", "data": str(userInsertedVal.inserted_id)}
                ),
                201,
            )
        else:
            return (
                jsonify(
                    {"status": "failure", "message": "Please enter another email id"}
                ),
                401,
            )
...
def login(collection):
    ...
    if "email" not in data or "password" not in data:
        return (
            jsonify({"status": "failure", "message": "enter complete information"}),
            400,
        )
    try:
        ...
        if user == None:
            return (
                jsonify(
                    {"status": "failure", "message": "Please enter correct credentials"}
                ),
                401,
            )
